  ...
  return {
    ...
    saveTask(taskForm: TaskForm, editingId?: string | null) {
      ...
      setData((currentData) => ({
        ...currentData,
        tasks: editingId
          ? currentData.tasks.map((item) =>
              item.id === editingId ? task : item,
            )
    ...
    saveApp(appForm: AppForm, editingId?: string | null) {
      const app: AppEntry = {
      ...
      setData((currentData) => ({
        ...currentData,
        apps: editingId
          ? currentData.apps.map((item) => (item.id === editingId ? app : item))
          : [app, ...currentData.apps],
      }))
    },
